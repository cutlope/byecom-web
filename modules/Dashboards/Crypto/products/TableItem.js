...
const apiurl = 'http://api.byecom.in/';

const TableItem = ({ fleet }) => {
  ...
  return (
    <React.Fragment>
      <TableRow className={clsx(classes.tableRowRoot, open ? 'active' : '')}>
        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellFirst)}>{fleet._id}</TableCell>
        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellFirst)}>
          <div>{new Date(fleet.createdAt).toLocaleString()} </div>
        </TableCell>

        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellSecond)}>
          <CmtObjectSummary
            avatar={<CmtAvatar src={apiurl + (fleet ? fleet?.goodsPhotos[0]?.path : " ")} alt={' '} />}
            title={fleet.goods}
        ...
        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellSecond)}>DA</TableCell>
        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellSecond)}>{fleet.fromAddress}</TableCell>
        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellSecond)}>{fleet.toAddress}</TableCell>
        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellSecond)}>₹{fleet.price}</TableCell>
        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellSecond)}>₹{fleet.cor}</TableCell>
        <TableCell className={clsx(classes.tableCellRoot, classes.tableCellSecond)}>
          {fleet.activeStatus ? '🟢 Active' : '🔴 Inactive'}
        </TableCell>
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            {/* <div className={classes.openDataRot}>
              ...
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <div className={'mr-3'}>
                  Total{' '}
                  <Box component="span" font="fontWeightRegular" color="text.primary">
                    Hours: {package.rate}
                  </Box>
                </div>
